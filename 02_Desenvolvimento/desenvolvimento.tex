...
Uma máscara binária de $73 \times 73$ foi gerada, indicando quais células do grid possuem interseção com o polígono municipal. Essa máscara é utilizada na etapa de avaliação para ignorar células que estão fora dos limites da cidade.
% TODO: Atualizar o número de células válidas (dentro do polígono) após reexecução do pipeline com $N = 73$. Com $N = 39$, eram 811 de 1.521.

\subsubsection{Agregação temporal}

As ocorrências foram contabilizadas por célula e por hora do ano, formando uma matriz esparsa \textit{célula} $\times$ \textit{hora}, para cada tipo de crime e para cada ano. Em seguida, essas matrizes horárias foram agregadas em janelas diárias (granularidade de 24h), resultando em sequências temporais com aproximadamente 365 passos por ano. As contagens dos 5 tipos de crime foram somadas e binarizadas (presença/ausência de crime: $\geq 1 \rightarrow 1$, caso contrário $\rightarrow 0$), produzindo um tensor final de dimensão $T \times 73 \times 73 \times 1$ (dias $\times$ linhas $\times$ colunas $\times$ canais) para Maceió, abrangendo 2012--2022.
% TODO: Confirmar o número exato de passos temporais $T$ após reexecução do pipeline com $N = 73$. Anteriormente, com $N = 39$, era $T = 4.018$.
