...
\label{eq:grid_size}
\end{equation}
\noindent em que $A_{\text{bbox}}$ é a área do \textit{bounding box} do polígono municipal (em km\textsuperscript{2}), calculada convertendo graus para quilômetros com correção de latitude, e $A_{\text{célula}} = 0{,}2$~km\textsuperscript{2}. Essa formulação torna o procedimento reutilizável para qualquer cidade --- basta substituir o polígono de entrada --- e reproduz, por construção, os valores de $N$ reportados em \citeonline{alborszumel2025} para as cidades estudadas (por exemplo, $N = 39$ para Baltimore e $N = 85$ para Chicago).
